...
LINEAR_MODEL_DICT = {
    ...
    "log10(Intracell. conc.)":0.485531592,
    "log10(metabolite value)":-0.940301993,
    "log10(charge)":0.461502729,
    "model RMSE":0.73,

}
INITIAL_OD = 0.03 # Per species
gDW_per_OD = 0.4 #https://bmcbiophys.biomedcentral.com/articles/10.1186/2046-1682-6-4/tables/1


INOCULUM_CFUs = {'At': {'mean': 1.871921e+07, 'std': 3.412908e+06}
                 'Ct': {'mean': 7.093596e+06, 'std': 7.819955e+05}
                 'Ml': {'mean': 5.320197e+07, 'std': 2.955665e+06}
                 'Oa': {'mean': 2.068966e+07, 'std': 5.911330e+06}}

CFU_TO_gDW = # https://ecmdb.ca/e_coli_stats 

def run(cs = 'Glucose'):
    growth_rates_df = pd.read_csv(FN_GROWTH_PARAMETERS, index_col=0)


def predict_leakage_rates(model):
